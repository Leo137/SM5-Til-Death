...
t[#t+1] = LoadFont("Common Normal")..{
	...
	SetCommand=function(self)
		if update then
			...
			-- self:GetParent():GetChild("Song Length"):x(math.min(((frameWidth-offsetX*2-150)/0.6)-5,self:GetWidth()*0.6+self:GetX()+5)):zoom(0.6)
		end
	end;
	CurrentSongChangedMessageCommand=function(self)
		self:queuecommand("Set")
...
t[#t+1] = LoadFont("Common Normal")..{
	Name="Song BPM";
	InitCommand=function(self)
		self:xy(frameX+offsetX+150,frameY+offsetY+130):zoom(0.3):halign(0):maxwidth((frameWidth-offsetX*2-150)/0.3)
	end;
	SetCommand=function(self)
		local song = GAMESTATE:GetCurrentSong()
		local bpms = {0,0}
...
t[#t+1] = LoadFont("Common Normal")..{
	...
	InitCommand=function(self)
		self:xy(frameX+offsetX+150,frameY+offsetY+145):zoom(0.3):halign(0):maxwidth((frameWidth-offsetX*2-150)/0.3)
	end;
	SetCommand=function(self)
		local song = GAMESTATE:GetCurrentSong()
		local bpms = {0,0}
